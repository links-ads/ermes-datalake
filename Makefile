...
fmt: check-venv			## Format code using black & isort.
	$(PY_BIN)/isort -v --src ckan/ --virtual-env $(PY_ENV)
	$(PY_BIN)/black ckan/

.PHONY: lint
lint: check-venv		## Run ruff, black, mypy (optional).
	@$(PY_BIN)/ruff check ckan/
	@$(PY_BIN)/black --check ckan/
	@if [ -x "$(PY_BIN)/mypy" ]; then $(PY_BIN)/mypy ckan/; else echo "mypy not installed, skipping"; fi
