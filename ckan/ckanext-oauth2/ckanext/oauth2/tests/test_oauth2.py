...
class OAuth2PluginTest(unittest.TestCase):
    ...
    def test_get_token_legacy_idm(self, OAuth2Session):
        ...
        expected_headers = {
            ...
            "Authorization": "Basic %s" % urlsafe_b64encode("%s:%s" % (helper.client_id, helper.client_secret)),
        }
    ...
    @httpretty.activate
    def test_identify(self, username, fullname=None, email=None, user_exists=True, fullname_field=True, sysadmin=None):
    ...
    @parameterized.expand(
        [({"error": "invalid_token", "error_description": "Error Description"},), ({"error": "another_error"},)]
    )
    @httpretty.activate
    ...
    def test_refresh_token(self, user_exists):
        ...
        if user_exists:
            ...
            session.refresh_token.assert_called_once_with(
                helper.token_endpoint, client_secret=helper.client_secret, client_id=helper.client_id, verify=True
            )
