...
class CloudStorage(object):
    def __init__(self):
        self.driver = get_driver(getattr(Provider, self.driver_name))(**self.driver_options)
    ...
    def container(self):
        ...
        if self._container is None:
            self._container = self.driver.get_container(container_name=self.container_name)
...
class ResourceCloudStorage(CloudStorage):
    def __init__(self, resource):
        ...
        if isinstance(upload_field_storage, (ALLOWED_UPLOAD_TYPES)) and upload_field_storage.filename:
        ...
        if self.filename:
            if self.can_use_advanced_azure:
                ...
                blob_service = azure_blob.BlockBlobService(
                    self.driver_options["key"], self.driver_options["secret"], protocol="https"
                )
            ...
            try:
                self.container.delete_object(self.container.get_object(self.path_from_filename(id, self.old_filename)))
            except ObjectDoesNotExistError:
    ...
    def get_url_from_filename(self, rid, filename, content_type=None):
        ...
        log.debug(f"use_secure_urls:{self.use_secure_urls}")
        log.debug(f"can_use_advanced_azure:{self.can_use_advanced_azure}")
        if self.can_use_advanced_azure:
            log.debug(f"adv az:{self.can_use_advanced_azure}")
        if self.use_secure_urls:
            log.debug(f"sec urls:{self.use_secure_urls}")
        ...
        if self.can_use_advanced_azure and self.use_secure_urls:
            ...
            s3_connection = S3Connection(
                self.driver_options["key"], self.driver_options["secret"], host=self.driver_options["host"]
            )

            if "region_name" in self.driver_options.keys():
                s3_connection.auth_region_name = self.driver_options["region_name"]

            generate_url_params = {"expires_in": 60 * 60, "method": "GET", "bucket": self.container_name, "key": path}
            if content_type:
        ...
        try:
            ...
            if "S3" in self.driver_name:
                return urljoin(
                    ...
                    "{container}/{path}".format(container=self.container_name, path=six.ensure_str(path)),
                )
