...
def fix_cors(domains):
    ...
    if cs.can_use_advanced_azure:
        ...
        blob_service = azure_blob.BlockBlobService(cs.driver_options["key"], cs.driver_options["secret"])

        blob_service.set_blob_service_properties(cors=[CorsRule(allowed_origins=domains, allowed_methods=["GET"])])
        return "Done!", True
    else:
        return (
...
def migrate(path, single_id):
    ...
    for i, resource in enumerate(iter(list(resources.items())), 1):
        ...
        print("[{i}/{count}] Working on {id}".format(i=i, count=len(resources), id=resource_id))
        try:
        ...
        with open(file_path, "rb") as fin:
            resource["upload"] = FakeFileStorage(fin, resource["url"].split("/")[-1])
            try:
    ...
    if failed:
        ...
        print("ID of all failed uploads are saved to `{0}`: {1}".format(log_file.name, failed))
