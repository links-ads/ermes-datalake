...
@pytest.mark.usefixtures("with_driver_options", "with_plugins", "with_request_context", "clean_db")
class TestMultipartUpload(object):
    ...
    def test_upload(self):
        ...
        multipart = helpers.call_action(
            "cloudstorage_initiate_multipart", id=res["id"], name="file.txt", size=1024 * 1024 * 5 * 2
        )
        storage = ResourceCloudStorage(res)
        assert storage.path_from_filename(res["id"], filename) == multipart["name"]
        ...
        result = helpers.call_action("cloudstorage_finish_multipart", uploadId=multipart["id"])
