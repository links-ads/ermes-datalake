...
class CSWHarvester(SpatialHarvester, SingletonPlugin):
    ...
    def get_original_url(self, harvest_object_id):
        ...
        url = urlunparse((parts.scheme, parts.netloc, parts.path, None, urlencode(params), None))
    ...
    def gather_stage(self, harvest_job):
        ...
        ids = []
        for guid in new:
            obj = HarvestObject(guid=guid, job=harvest_job, extras=[HOExtra(key="status", value="new")])
            obj.save()
            ids.append(obj.id)
        for guid in change:
            obj = HarvestObject(
