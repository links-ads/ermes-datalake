...
class WAFHarvester(SpatialHarvester, SingletonPlugin):
    ...
    def get_original_url(self, harvest_object_id):
        url = (
            model.Session.query(HOExtra.value)
    ...
    def gather_stage(self, harvest_job, collection_package_id=None):
        ...
        query = (
            model.Session.query(HarvestObject.guid, HarvestObject.package_id, HOExtraAlias1.value, HOExtraAlias2.value)
        ...
        def create_extras(url, date, status):
            ...
            if collection_package_id:
                extras.append(HOExtra(key="collection_package_id", value=collection_package_id))
            return extras
        ...
        for location in new:
            ...
            obj = HarvestObject(
                job=harvest_job, extras=create_extras(location, url_to_modified_harvest[location], "new"), guid=guid
            )
            obj.save()
            ids.append(obj.id)
        ...
        for location in change:
            obj = HarvestObject(
                job=harvest_job,
                extras=create_extras(location, url_to_modified_harvest[location], "change"),
        ...
        for location in delete:
            ...
            model.Session.query(HarvestObject).filter_by(guid=url_to_ids[location][0]).update(
                {"current": False}, False
            )
    ...
    def fetch_stage(self, harvest_object):
        ...
        if not url:
            self._save_object_error("No location defined for object {0}".format(harvest_object.id), harvest_object)
            return False
        ...
        if document_format == "iso":
            ...
            extra = HOExtra(object=harvest_object, key="original_document", value=content)
            extra.save()

            extra = HOExtra(object=harvest_object, key="original_format", value=document_format)
            extra.save()
...
apache = (
    ...
    + parse.Optional(
        parse.Combine(
            parse.Word(parse.alphanums + "-") + parse.Word(parse.alphanums + ":"), adjacent=False, joinString=" "
        ).setResultsName("date")
    )
)
...
other = parse.SkipTo(parse.CaselessLiteral("<a href="), include=True).suppress() + parse.quotedString.setParseAction(
    parse.removeQuotes
).setResultsName("url")
