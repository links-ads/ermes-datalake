...
class DocHarvester(SpatialHarvester, SingletonPlugin):
    ...
    def gather_stage(self, harvest_job):
        ...
        def create_extras(url, status):
            return [HOExtra(key="doc_location", value=url), HOExtra(key="status", value=status)]

        if not existing_object:
            guid = hashlib.md5(url.encode("utf8", "ignore")).hexdigest()
            harvest_object = HarvestObject(job=harvest_job, extras=create_extras(url, "new"), guid=guid)
        else:
