...
def text_traceback():
    with warnings.catch_warnings():
        ...
        res = "the original traceback:".join(cgitb.text(sys.exc_info()).split("the original traceback:")[1:]).strip()
    return res
...
class SpatialHarvester(HarvesterBase):
        ...
        if len(iso_values["bbox"]) > 0:
            ...
            try:
                ...
                self._save_object_error(
                    "Error parsing bounding box value: {0}".format(six.text_type(e)), harvest_object, "Import"
                )
            else:
                ...
                if xmin == xmax or ymin == ymax:
                    extent_string = Template('{"type": "Point", "coordinates": [$x, $y]}').substitute(x=xmin, y=ymin)
                    self._save_object_error("Point extent defined instead of polygon", harvest_object, "Import")
                else:
                    extent_string = self.extent_template.substitute(xmin=xmin, ymin=ymin, xmax=xmax, ymax=ymax)

                extras["spatial"] = extent_string.strip()
        else:
            log.debug("No spatial extent defined for this object")

        resource_locators = iso_values.get("resource-locator", []) + iso_values.get(
    ...
    def import_stage(self, harvest_object):
        ...
        if original_document and original_format:
            ...
            if harvest_object.content is None:
                self._save_object_error(
                    "Empty content for object {0}".format(harvest_object.id), harvest_object, "Import"
                )
                return False
            ...
            if not is_valid:
                ...
                continue_import = p.toolkit.asbool(
                    config.get("ckanext.spatial.harvest.continue_on_validation_errors", False)
                ) or self.source_config.get("continue_on_validation_errors")
        ...
        try:
            ...
            self._save_object_error(
                "Error parsing ISO document for object {0}: {1}".format(harvest_object.id, six.text_type(e)),
                harvest_object,
                "Import",
            )
            return False
        ...
        if iso_guid and harvest_object.guid != iso_guid:
            ...
            if existing_object:
                self._save_object_error(
                    "Object {0} already has this guid {1}".format(existing_object.id, iso_guid),
                    harvest_object,
                    "Import",
                )
                return False
        ...
        context.update({"extras_as_string": True, "api_version": "2", "return_id_only": True})
        ...
        if status == "new":
            ...
            try:
                ...
                self._save_object_error(
                    "Validation Error: %s" % six.text_type(e.error_summary), harvest_object, "Import"
                )
                return False
            ...
            if (
                ...
                # harvest object
                if (
                    config.get("ckanext.spatial.harvest.reindex_unchanged", True) != "False"
    ...
    def _get_validator(self):
        ...
        if not hasattr(self, "_validator"):
            if hasattr(self, "source_config") and self.source_config.get("validator_profiles", None):
                ...
                profiles = [x.strip() for x in config.get("ckan.spatial.validator.profiles").split(",")]
            else:
    ...
    def _validate_document(self, document_string, harvest_object, validator=None):
        ...
        if not valid:
            log.error(
                "Validation errors found using profile {0} for object with GUID {1}".format(
                    profile, harvest_object.guid
                )
            )
            for error in errors:
