        ...
        try:
            ...
            if not six.text_type(e).strip():
                ...
                self._save_object_error(
                    "Error importing Gemini document: %s" % six.text_type(e), harvest_object, "Import"
                )
            raise
    ...
    def write_package_from_gemini_string(self, content):
        ...
        try:
            ...
            try:
                ...
                raise Exception(
                    "Could not extract reference date for GUID %s (%s)" % (gemini_guid, gemini_values["metadata-date"])
                )

        self.obj.metadata_modified_date = metadata_modified_date
        self.obj.save()

        last_harvested_object = (
        ...
        if last_harvested_object:
            ...
            # needs to be updated
            if (
                last_harvested_object.metadata_modified_date is None
                ...
                or (
                    ...
                    and last_harvested_object.source.active is False
                )
            ):
                ...
                if (
                    last_harvested_object.content != self.obj.content
                    and last_harvested_object.metadata_modified_date == self.obj.metadata_modified_date
                ):
        ...
        if len(resource_locators):
                    ...
                    resource.update(
                        {
                            ...
                            "description": (
                                ...
                                if resource_locator.get("description")
            ...
            verified_view_resources = [
                r for r in package_dict["resources"] if "verified" in r and r["format"] == "WMS"
            ]
    ...
    def get_gemini_string_and_guid(self, content, url=None):
        ...
        if gemini_xml is None:
            self._save_gather_error(
                "Content is not a valid Gemini document without the gmd:MD_Metadata element", self.harvest_job
            )
...
class GeminiCswHarvester(GeminiHarvester, SingletonPlugin):
    ...
    def gather_stage(self, harvest_job):
        ...
        try:
            ...
            self._save_gather_error(
                "Error gathering the identifiers from the CSW server [%s]" % six.text_type(e), harvest_job
            )
            return None
    ...
    def fetch_stage(self, harvest_object):
        ...
        try:
            ...
            self._save_object_error(
                "Error saving the harvest object for GUID %s [%r]" % (identifier, e), harvest_object
            )
            return False
    ...
    def gather_stage(self, harvest_job):
        ...
        try:
            ...
            if gemini_guid:
                ...
                obj = HarvestObject(guid=gemini_guid, job=harvest_job, content=gemini_string)
                obj.save()
            ...
            self._save_gather_error(
                "Error parsing the document. Is this a valid Gemini document?: %s [%r]" % (url, e), harvest_job
            )
            if debug_exception_mode:
                raise
            return None
