...
def _bbox_2_wkt(bbox, srid):
    ...
    wkt = bbox_template.substitute(minx=bbox["minx"], miny=bbox["miny"], maxx=bbox["maxx"], maxy=bbox["maxy"])
...
def bbox_query_ordered(bbox, srid=None):
    ...
    params = {"query_bbox": six.text_type(input_geometry), "query_srid": input_geometry.srid}
    ...
    log.debug("Spatial results: %r", [("%.2f" % extent.spatial_ranking, extent.package_id) for extent in extents[:20]])
    return extents
