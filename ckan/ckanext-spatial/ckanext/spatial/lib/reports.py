...
def validation_report(package_id=None):
    ...
    query = model.Session.query(HarvestObject).filter_by(current=True).order_by(HarvestObject.fetch_finished.desc())
    ...
    for harvest_object in query:
        ...
        for err in harvest_object.errors:
            if "not a valid Gemini" in err.message or "Validating against" in err.message:
