...
def validate_file(metadata_filepath):
    ...
    try:
        ...
        print("ERROR: Unicode Error reading file '%s': %s" % (metadata_filepath, e))
        sys.exit(1)
...
def update_extents():
    ...
    packages = [extra.package for extra in Session.query(PackageExtra).filter(PackageExtra.key == "spatial").all()]
    ...
    for package in packages:
        try:
            ...
            errors.append("Package %s - Error decoding JSON object: %s" % (package.id, six.text_type(e)))
