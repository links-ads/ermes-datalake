...
class TestSpatialExtra(SpatialTestBase):
    def test_spatial_extra_base(self, app):
        ...
        package_extent = Session.query(PackageExtent).filter(PackageExtent.package_id == dataset["id"]).first()
        ...
        assert Session.query(func.ST_X(package_extent.the_geom)).first()[0] == geojson["coordinates"][0]
        assert Session.query(func.ST_Y(package_extent.the_geom)).first()[0] == geojson["coordinates"][1]
    ...
    def test_spatial_extra_edit(self, app):
        ...
        assert Session.query(func.ST_GeometryType(package_extent.the_geom)).first()[0] == "ST_Polygon"
        assert package_extent.the_geom.srid == self.db_srid
    ...
    def test_spatial_extra_bad_json(self, app):
        ...
        if tk.check_ckan_version(min_version="2.9"):
            data = {"name": dataset["name"], "extras__0__key": "spatial", "extras__0__value": '{"Type":Bad Json]'}
            res = app.post(offset, environ_overrides=env, data=data)
        else:
            form = res.forms[1]
            form["extras__0__key"] = "spatial"
    ...
    def test_spatial_extra_bad_geojson(self, app):
        ...
        if tk.check_ckan_version(min_version="2.9"):
            data = {
                "name": dataset["name"],
                "extras__0__key": "spatial",
                "extras__0__value": '{"Type":"Bad_GeoJSON","a":2}',
            }
            res = app.post(offset, environ_overrides=env, data=data)
        else:
            form = res.forms[1]
