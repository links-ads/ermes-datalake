...
if p.toolkit.check_ckan_version(min_version="2.9"):
    ...
    from ckanext.spatial.plugin.flask_plugin import SpatialQueryMixin, HarvestMetadataApiMixin
else:
    from pylons import config
    from ckanext.spatial.plugin.pylons_plugin import SpatialQueryMixin, HarvestMetadataApiMixin
            ...
            if extra.key == "spatial":
                if extra.state == "active" and extra.value:
                    try:
                        ...
                        error_dict = {"spatial": ["Error decoding JSON object: %s" % six.text_type(e)]}
                    ...
                    try:
                        ...
                        error_dict = {"spatial": ["Error creating geometry: %s" % six.text_type(e)]}
                        raise p.toolkit.ValidationError(error_dict, error_summary=package_error_summary(error_dict))
                    except Exception as e:
                        if bool(os.getenv("DEBUG")):
                            raise
                        error_dict = {"spatial": ["Error: %s" % six.text_type(e)]}
                        raise p.toolkit.ValidationError(error_dict, error_summary=package_error_summary(error_dict))
...
class SpatialQuery(SpatialQueryMixin, p.SingletonPlugin):
        ...
        if pkg_dict.get("extras_spatial", None) and self.search_backend in ("solr", "solr-spatial-field"):
            ...
            if self.search_backend == "solr":
                ...
                if not (
                    ...
                    log.error(
                        "Solr backend only supports bboxes (Polygons with 5 points), ignoring geometry {0}".format(
                            pkg_dict["extras_spatial"]
                        )
                    )
                    return pkg_dict
                ...
                pkg_dict["bbox_area"] = (pkg_dict["maxx"] - pkg_dict["minx"]) * (pkg_dict["maxy"] - pkg_dict["miny"])
                ...
                if not wkt:
                    ...
                    if not shape.is_valid:
                        log.error("Wrong geometry, it mays contain overlapping polygons")
    ...
    def _params_for_solr_spatial_field_search(self, bbox, search_params):
        ...
        search_params["fq_list"].append(
            '+spatial_geom:"Intersects(ENVELOPE({minx}, {maxx}, {maxy}, {miny}))"'.format(
    ...
    def _params_for_postgis_search(self, bbox, search_params):
        ...
        if search_params.get("sort") == "spatial desc" and p.toolkit.asbool(
            ...
            search_params["extras"]["ext_spatial"] = [
                (extent.package_id, extent.spatial_ranking) for extent in extents[start : start + rows]
            ]
        else:
    ...
    def after_search(self, search_results, search_params):
        ...
        # Note: This will be deprecated at some point in favour of the
        # Solr 4 spatial sorting capabilities
        if search_params.get("extras", {}).get("ext_spatial") and p.toolkit.asbool(
            config.get("ckanext.spatial.use_postgis_sorting", "False")
        ):
