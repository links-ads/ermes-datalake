...
class ISO19139Schema(XsdValidator):
    ...
    def is_valid(cls, xml):
        ...
        gmx_xsd_filepath = os.path.join(os.path.dirname(__file__), xsd_path, "gmx/gmx.xsd")
...
class ISO19139EdenSchema(XsdValidator):
    ...
    def is_valid(cls, xml):
        ...
        if metadata_type in ("dataset", "series"):
            ...
            gmx_and_srv_xsd_filepath = os.path.join(os.path.dirname(__file__), xsd_path, "gmx_and_srv.xsd")
            ...
            is_valid = False
            errors = [
                ('Metadata type not recognised "%s" - cannot choose an ISO19139 validator.' % metadata_type, None)
            ]
        if is_valid:
            return True, []
...
class ISO19139NGDCSchema(XsdValidator):
    ...
    def is_valid(cls, xml):
        ...
        xsd_filepath = os.path.join(os.path.dirname(__file__), xsd_path, "schema.xsd")
...
class FGDCSchema(XsdValidator):
    ...
    def is_valid(cls, xml):
        ...
        xsd_filepath = os.path.join(os.path.dirname(__file__), xsd_path, "fgdc-std-001-1998.xsd")
...
class ConstraintsSchematron(SchematronValidator):
    ...
    @classmethod
    def get_schematrons(cls):
        with resource_stream(__name__, "xml/medin/ISOTS19139A1Constraints_v1.3.sch") as schema:
...
class ConstraintsSchematron14(SchematronValidator):
    ...
    @classmethod
    def get_schematrons(cls):
        with resource_stream(__name__, "xml/medin/ISOTS19139A1Constraints_v1.4.sch") as schema:
            return [cls.schematron(schema)]
...
class Gemini2Schematron(SchematronValidator):
    ...
    @classmethod
    def get_schematrons(cls):
        with resource_stream(__name__, "xml/gemini2/gemini2-schematron-20110906-v1.2.sch") as schema:
            return [cls.schematron(schema)]
...
class Gemini2Schematron13(SchematronValidator):
    ...
    @classmethod
    def get_schematrons(cls):
        with resource_stream(__name__, "xml/gemini2/Gemini2_R1r3.sch") as schema:
            return [cls.schematron(schema)]
