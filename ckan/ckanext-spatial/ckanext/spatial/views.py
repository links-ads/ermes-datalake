...
def spatial_query(register):
    ...
    srid = get_srid(request.args.get("crs")) if "crs" in request.args else None
...
def _get_xslt(original=False):
    ...
    if xslt:
        if ":" in xslt:
            ...
            log.error("XSLT should be defined in the form <package>:<path>" + ", eg ckanext.myext:templates/my.xslt")
...
harvest_metadata.add_url_rule("/api/2/rest/harvestobject/<id>/xml", view_func=harvest_object_redirect_xml)
harvest_metadata.add_url_rule("/api/2/rest/harvestobject/<id>/html", view_func=harvest_object_redirect_html)

harvest_metadata.add_url_rule("/harvest/object/<id>/original", view_func=display_xml_original)
harvest_metadata.add_url_rule("/harvest/object/<id>/html", view_func=display_html)
harvest_metadata.add_url_rule("/harvest/object/<id>/html/original", view_func=display_html_original)
