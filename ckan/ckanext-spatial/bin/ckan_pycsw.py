...
def setup_db(pycsw_config):
    ...
    pycsw.core.admin.setup_db(database, table_name, "", create_plpythonu_functions=False, extra_columns=ckan_columns)
...
def load(pycsw_config, ckan_url):
    ...
    log.info("Gather finished ({0} datasets): {1}".format(len(gathered_records.keys()), str(datetime.datetime.now())))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--pycsw_config", action="store", default="default.cfg", help="pycsw config file to use."
    )

    parser.add_argument("-u", "--ckan_url", action="store", help="CKAN instance to import the datasets from.")
