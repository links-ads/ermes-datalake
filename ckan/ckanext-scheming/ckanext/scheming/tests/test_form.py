...
def _get_package_update_page_as_sysadmin(app, id):
    user = Sysadmin()
    env = {"REMOTE_USER": user["name"].encode("ascii")}
    response = app.get(url="/test-schema/edit/{}".format(id), extra_environ=env)
    return env, response
...
def _get_resource_new_page_as_sysadmin(app, id):
    user = Sysadmin()
    env = {"REMOTE_USER": user["name"].encode("ascii")}
    response = app.get(url="/dataset/new_resource/{}".format(id), extra_environ=env)
    return env, response
...
class TestGroupFormNew(object):
    @pytest.mark.skipif(not ckantoolkit.check_ckan_version(min_version="2.7.0"), reason="Unspecified")
...
class TestCustomOrgFormNew(object):
    ...
    def test_org_form_includes_custom_field(self, app):
        env, response = _get_organization_new_page_as_sysadmin(app, type="publisher")
        form = BeautifulSoup(response.body).select("form")[1]
...
class TestJSONDatasetForm(object):
    ...
    def test_dataset_form_update(self, app):
        ...
        env, response = _get_package_update_page_as_sysadmin(app, dataset["id"])
        form = BeautifulSoup(response.body).select_one("#dataset-edit")
        assert form.select_one("textarea[name=a_json_field]").text == json.dumps(value, indent=2)
...
class TestJSONResourceForm(object):
    ...
    def test_resource_form_update(self, app):
        ...
        env, response = _get_resource_update_page_as_sysadmin(app, dataset["id"], dataset["resources"][0]["id"])
        form = BeautifulSoup(response.body).select_one("#resource-edit")
        assert form.select_one("textarea[name=a_resource_json_field]").text == json.dumps(value, indent=2)
        ...
        if not url.startswith("/"):  # ckan < 2.9
            url = "/dataset/{ds}/resource_edit/{rs}".format(ds=dataset["id"], rs=dataset["resources"][0]["id"])
...
class TestSubfieldDatasetForm(object):
    ...
    def test_dataset_form_update(self, app):
        ...
        env, response = _get_package_update_page_as_sysadmin(app, dataset["id"])
        form = BeautifulSoup(response.body).select_one("#dataset-edit")
        assert form.select_one("input[name=citation-1-originator]").attrs["value"] == "ahmed"
