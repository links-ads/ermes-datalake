...
class _GroupOrganizationMixin(object):
    ...
    def validate(self, context, data_dict, schema, action):
        ...
        if not t or t not in self._schemas:
            return data_dict, {"type": "Unsupported {thing} type: {t}".format(thing=thing, t=t)}
        scheming_schema = self._expanded_schemas[t]
        scheming_fields = scheming_schema["fields"]

        get_validators = _field_output_validators_group if action_type == "show" else _field_validators
        for f in scheming_fields:
            schema[f["field_name"]] = get_validators(f, scheming_schema, f["field_name"] not in schema)
...
class SchemingGroupsPlugin(p.SingletonPlugin, _GroupOrganizationMixin, DefaultGroupForm, _SchemingMixin):
...
class SchemingOrganizationsPlugin(p.SingletonPlugin, _GroupOrganizationMixin, DefaultOrganizationForm, _SchemingMixin):
    p.implements(p.IConfigurer)
    p.implements(p.ITemplateHelpers)
    p.implements(p.IGroupForm, inherit=True)
    p.implements(p.IActions)
...
def _field_output_validators_group(f, schema, convert_extras):
    ...
    return _field_output_validators(
        f, schema, convert_extras, convert_from_extras_type=validation.convert_from_extras_group
    )


def _field_output_validators(f, schema, convert_extras, convert_from_extras_type=convert_from_extras):
    """
    Return the output validators for a scheming field f
    """
    if "repeating_subfields" in f:
        validators = {sf["field_name"]: _field_output_validators(sf, schema, False) for sf in f["repeating_subfields"]}
    elif convert_extras:
        validators = [convert_from_extras_type, ignore_missing]
    else:
        validators = [ignore_missing]
    if "output_validators" in f:
        validators += validation.validators_from_string(f["output_validators"], f, schema)
    return validators
...
def _field_validators(f, schema, convert_extras):
    ...
    # them.
    if "repeating_subfields" in f:
        validators = {sf["field_name"]: _field_validators(sf, schema, False) for sf in f["repeating_subfields"]}
...
def _field_create_validators(f, schema, convert_extras):
    ...
    # them.
    if "repeating_subfields" in f:
        validators = {sf["field_name"]: _field_create_validators(sf, schema, False) for sf in f["repeating_subfields"]}
            ...
            for field in schema[grouping]:
                if "repeating_subfields" in field:
                    field["repeating_subfields"] = [
                        _expand(schema, subfield) for subfield in field["repeating_subfields"]
                    ]
                elif "simple_subfields" in field:
                    field["simple_subfields"] = [_expand(schema, subfield) for subfield in field["simple_subfields"]]
...
def create_ontology():
    ...
    vocab_file = config.get("scheming.ontology_schema", None)
    if vocab_file:
        vocabulary = _load_schema(vocab_file)
        try:
            data = {"id": vocabulary["name"]}
            tk.get_action("vocabulary_show")(context, data)
            log.info("Vocabulary already exists, skipping.")
        except tk.ObjectNotFound:
            log.info(f"Creating vocab {vocabulary['name']}")
            vocab = tk.get_action("vocabulary_create")(context, vocabulary)
...
class SchemingDatasetsPlugin(p.SingletonPlugin, DefaultDatasetForm, _SchemingMixin):
    ...
    def validate(self, context, data_dict, schema, action):
        ...
        if before:
            schema["__before"] = validation.validators_from_string(before, None, scheming_schema)
        if after:
            schema["__after"] = validation.validators_from_string(after, None, scheming_schema)
        fg = (
        ...
        for field_list, destination, convert_extras in fg:
            for f in field_list:
                ...
                destination[f["field_name"]] = get_validators(f, scheming_schema, convert_this)
        ...
        if action_type == "show":
            ...
            dataset_composite = {
                f["field_name"] for f in scheming_schema["dataset_fields"] if "repeating_subfields" in f
            }
            if dataset_composite:
                expand_form_composite(data_dict, dataset_composite)
            resource_composite = {
                f["field_name"] for f in scheming_schema["resource_fields"] if "repeating_subfields" in f
            }
            ...
            if composite_convert_fields:
                schema = dict(schema, __after=schema.get("__after", []) + [composite_convert_to])
