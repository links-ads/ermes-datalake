...
def scheming_datastore_choices(field):
    ...
    try:
        result = lc.action.datastore_search(resource_id=resource_id, limit=limit, fields=fields)
    ...
    datastore_choices = [{"value": r[fields[0]], "label": r[fields[1]]} for r in result["records"]]
...
def date_tz_str_to_datetime(date_str):
    ...
    if len(time_tuple) >= 6:
        ...
        if not m:
            raise ValueError("Unable to parse %s as seconds.microseconds" % time_tuple[5])
