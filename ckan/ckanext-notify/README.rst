...
* A CKAN Extension "plugin" at ``ckanext/notify/plugin.py`` which, when
...
To install this package, from your CKAN virtualenv, run the following from your CKAN base folder (e.g. ``pyenv/``)::
  ...
  pip install -e git+https://github.com/okfn/ckanext-notify#egg=ckanext-notify

Then activate it by setting ``ckan.plugins = notify`` in your main ``ini``-file.

Orientation
===========

* Examine the source code, starting with ``ckanext/notify/plugin.py``
...
* These are set to point at directories within
  ``ckanext/notify/theme/`` (in this package).  Here we:
   * override the home page HTML ``ckanext/notify/theme/templates/home/index.html``
...
* The custom package edit form at ``package_form.py`` follows a deprecated
  way to make a form (using FormAlchemy). This part of the notify Theme needs
  updating. In the meantime, follow the instructions at: 
  http://docs.ckan.org/en/latest/forms.html

notify Tags With Vocabularies
==============================

To add notify tag vocabulary data to the database, from the ckanext-notify directory run:

::

    paster notify create-notify-vocabs -c <path to your ckan config file>

This data can be removed with

::

    paster notify clean -c <path to your ckan config file>
